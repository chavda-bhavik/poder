...
const Episode: React.FC<EpisodeProps> = ({
    ...
    if (id === selectedEpisodeId) {
        ...
        else actionIcon = <PlayIcon onClick={() => resume()} className="text-maroon w-10 inline" />;
    }
    return (
        <motion.div
            initial="hidden"
        ...
        >
            <div className="flex flex-row rounded overflow-hidden border shadow-md">
                <img className="block h-auto w-24 lg:w-20 bg-cover" src={imageSrc} alt={imageAlt} />
                <div className="bg-gray-200 p-2 w-full flex flex-col lg:flex-row justify-between leading-normal">
                    <div className="flex justify-center flex-col mb-1">
                        ...
                        {/* <p className="text-gray-500 text-sm">{dayjs(date).format('MMMM DD, YYYY')}</p> */}
                    </div>
